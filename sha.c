...
//This is a array of functions having constants values of uncigned character type
const uint32_t k[64] = {
...
static int calc_chunk(uint8_t chunk[CHUNK_SIZE], struct buffer_state * state)
{
	...
	state->p += state->len;
	state->len = 0;
    printf("%d-%d-%d--%d\n",chunk,*state->p,state->len,space_in_chunk);
